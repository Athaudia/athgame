...
uint8_t age_ehbasic__cpu_read_mem(void* s, uint16_t pos)
{
...
void age_ehbasic__cpu_write_mem(void* s, uint16_t pos, uint8_t val)
{
	struct age_ehbasic* sys = (struct age_ehbasic*)s;
	if(pos == 0xF001)
	{
...
struct age_ehbasic* age_ehbasic__new()
{
	...
	sys->cpu = age_6502__new(sys, age_ehbasic__cpu_read_mem, age_ehbasic__cpu_write_mem);
	sys->str = 0;
	sys->strpos = 0;
	return sys;
}
